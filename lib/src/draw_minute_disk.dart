...
void drawMinuteDisk(
  ...
  // Draw the clock disc
  canvas.drawCircle(center, size.width / 4, clockDiscPaint);
  // Draw outer border
  final double tickLength = 20;
  final double weightedRadius = (size.width + size.height) / 5.5;
  ...
  for (int tick = 0; tick < 60; tick++) {
    double angle =
        ((tick * 2 * math.pi) / 60) + minute * (2 * math.pi / 60) - math.pi / 2;
    ...
    textPainter.paint(
      ...
      Offset(
        center.dx +
            (weightedRadius - 27) * math.cos(angle) -
            textPainter.width / 2,
        center.dy +
            (weightedRadius - 27) * math.sin(angle) -
